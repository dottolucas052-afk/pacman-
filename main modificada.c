...
int main() {
    ...
    float pacman_rotacao = 0.0f; // MODIFICAÇÃO (Inicializa a rotação)
    // Conta portais (T)
    int contadorT = 0;
    for (int i = 0; i < 20; i++)
        for (int j = 0; j < 40; j++)
    ...
    for (int i = 0; i < 20; i++) {
        for (int j = 0; j < 40; j++) {
            if (mapa[i][j] == 'F') {
                ...
                // --- 1. INFORMA QUE O FANTASMA COMEÇA VIVO ---
                array_fantasmas[qnt_f-1].esta_vivo = true; 
    ...
    // MODIFICAÇÃO (Assumindo que estão na pasta sprites)
    ...
    textura_teleporte = LoadTexture("sprites/teleporte.png"); 
    textura_logo = LoadTexture("sprites/logo.png"); 
 ...
 // Níveis
    ...
    while (!WindowShouldClose()) {
        ...
        if (IsKeyPressed(KEY_R)) {
            ...
            if (controle[i][j] == 'F') {
                ...
                // --- 2. RECARREGA O FANTASMA COMO VIVO ---
                array_fantasmas[qnt_f - 1].esta_vivo = true;
    ...
    if (venceu) {
        ...
        // Zera contadores antes de recarregar
        pellets = 0;
        free(array_fantasmas);
        array_fantasmas = NULL;
        qnt_f = 0;
        
        int qnt = 0; // --- 3. CORREÇÃO (Declarar qnt aqui fora) ---
    
        for (int i = 0; i < 20; i++) { // --- 3. CORREÇÃO (Remover 'qnt=0' daqui) ---
            for (int j = 0; j < 41; j++) {
                mapa[i][j] = controle[i][j];
                if (controle[i][j] == '.' || controle[i][j] == 'o') {
                    pellets++;
                ...
                if(controle[i][j] == 'F'){
                    qnt++;
                    // array_fantasmas = realloc(array_fantasmas, qnt_f * sizeof(tipo_objeto)); // --- 4. BUG ESTAVA AQUI ---
                    array_fantasmas = realloc(array_fantasmas, qnt * sizeof(tipo_objeto)); // --- 4. CORREÇÃO ---
                    ...
                    // --- 5. RECARREGA O FANTASMA COMO VIVO ---
                    array_fantasmas[qnt-1].esta_vivo = true;
        ...
        qnt_f = qnt; // --- 6. CORREÇÃO (Atualiza a contagem global) ---
        power_up_ativo = false;
        // DrawRectangle(0, 0, 20, 20, BLACK); // (Isso desenha um quadrado preto no canto)
        venceu = false;
        continue;
    }
        ...
        for (int i = 0; i < qnt_f; i++) {
            ...
            // --- 7. VERIFICA SE O FANTASMA ESTÁ VIVO ANTES DE DESENHAR ---
            if (array_fantasmas[i].esta_vivo) {

                float aux_fantasma_x, aux_fantasma_y;
                Rectangle origem_f;
                // Rectangle destino_f = { aux_fantasma_x, aux_fantasma_y, (float)CELULA, (float)CELULA }; // --- 8. BUG (Usado antes de definir aux_x/y) ---
                ...
                Rectangle destino_f = { aux_fantasma_x, aux_fantasma_y, (float)CELULA, (float)CELULA }; // --- 8. CORREÇÃO (Movido para cá) ---
        ...
        if (pacman.teleportado) {
            ...
            // --- 9. CORREÇÃO DE BUG CRÍTICO (usava array_fantasmas[i] para desenhar o pacman) ---
         ...
         if (jogo_pausado) {
            ...
            if (IsKeyPressed(KEY_N)) {
                ...
                if (controle[i][j] == 'F') {
                    ...
                    // --- 10. RECARREGA O FANTASMA COMO VIVO (NO MENU 'N') ---
                    array_fantasmas[qnt_f - 1].esta_vivo = true; 
        ...
        if(contador_pacman >= intervalo_pacman){
            ...
            if (!jogo_pausado) {
                ...
                if (mapa[x][y] == '.') {
                    pellets--;
                    // mapa[i][j] = ' '; // --- 11. BUG CRÍTICO (i e j não existem aqui) ---
                    mapa[x][y] = ' '; // --- 11. CORREÇÃO ---
                    pontos += 10;
                    if(pellets == 0 && !venceu){
                        nivel++;
                        venceu = true;
                    ...
                    // mapa[i][j] = ' '; // --- 12. BUG CRÍTICO (i e j não existem aqui) ---
                    mapa[x][y] = ' '; // --- 12. CORREÇÃO ---
                    pellets--;
                    power_up_ativo = true;
        ...
        if(contador_fantasmas >= intervalo_fantasmas){
                ...
                for (int i = 0; i < qnt_f; i++) {
                    // --- 13. SÓ ATUALIZA SE ESTIVER VIVO ---
                    if (array_fantasmas[i].esta_vivo) {
                ...
                for(int i = 0; i< qnt_f; i++){
                    // --- 14. SÓ ATUALIZA SE ESTIVER VIVO ---
                    if (array_fantasmas[i].esta_vivo) {
    ...
    // ~~~~ 6) Libera memória ~~~~ //
    CloseWindow();
    UnloadTexture(textura_pacman); // --- 16. ADICIONADO (Boa prática) ---
    UnloadTexture(textura_fantasma_normal); // --- 16. ADICIONADO (Boa prática) ---
    UnloadTexture(textura_fantasma_nervoso); // --- 16. ADICIONADO (Boa prática) ---
    UnloadTexture(textura_teleporte);
    UnloadTexture(textura_logo); // --- 16. ADICIONADO (Boa prática) ---
    free(array_fantasmas);
    free(portais);
    return 0;
