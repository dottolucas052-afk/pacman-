    ...
    for (int i = 0; i < 20; i++) {
        for (int j = 0; j < 40; j++) {
            if (mapa[i][j] == 'F') {
                ...
                array_fantasmas[qnt_f - 1].direcao_atual = CIMA; // inicialização padrão
...
// ~~~~ movimento ~~~~ //
        if(IsKeyDown(KEY_UP))x--;
        if(IsKeyDown(KEY_DOWN))x++;
        if(IsKeyDown(KEY_RIGHT))y++;
        if(IsKeyDown(KEY_LEFT))y--;
        
        
// ~~~~ controle de colisão ~~~~/
        if (x < 0 || x >= 20 || y < 0 || y >= 40) { x = aux1; y = aux2; }
        if(mapa[x][y] == '#'){
            x = aux1;}
        if(mapa[x][y] == '.'){mapa[x][y] = ' ';}
        if(mapa[x][y] == 'o'){mapa[x][y] = ' ';}
        
        
// ~~~~ movimento dos fantasmas ~~~~ /
        for (int i = 0; i < qnt_f; i++) {
            // ---- CHAMADA SEM CAST (assinatura usa [20][41]) ----
        ...
        for (int i = 0; i < qnt_f; i++) {
            DrawRectangle(array_fantasmas[i].posicao.coluna * 20, array_fantasmas[i].posicao.linha * 20, 20, 20, PURPLE);
        }
        DrawCircle(y*20 + 10, x*20 + 10, 9, GOLD);
